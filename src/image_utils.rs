...
impl ImageSearchCache {
    ...
    pub fn insert(&mut self, img: Image) {
        ...
        self.path_map
            .insert(rc_img.borrow().path.clone(), rc_img.clone());
        self.phash_map
            .insert(rc_img.borrow().percept_hash.clone(), rc_img.clone());
        self.fhash_map
            .insert(rc_img.borrow().file_hash.clone(), rc_img.clone());

        for kw in &rc_img.borrow().keywords {
            match self.keyword_map.entry(kw.clone()) {
                Entry::Occupied(o) => o.into_mut().push(rc_img.clone()),
                Entry::Vacant(v) => {
                    v.insert(vec![rc_img.clone()]);
                }
            }
        }
    ...
    pub fn write_to_file<T: AsRef<Path>>(&self, path: T) -> Result<()> {
        ...
        let v: Vec<Image> = self.inner.iter().map(|img| img.borrow().clone()).collect();
